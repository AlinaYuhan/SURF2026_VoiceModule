...
bash -n "${WORKSPACE_ROOT}/scripts/run_demo_server.sh"
...
for path in (
    ...
    Path("demo_server.py"),
    Path("demo_surf_context_node.py"),
...
if [[ "${QWEN_AUTOSTART_ASR_BRIDGE}" != "0" ]]; then
  echo "QWEN_AUTOSTART_ASR_BRIDGE must be 0 for this integrated workspace." >&2
  exit 1
fi

echo "SURF -> Demo workspace check passed."
