...
class DemoSurfContextNode(Node):
    """Consume SURF context topics and run the demo reply/TTS/action backend."""

    def __init__(self) -> None:
        super().__init__("demo_surf_context_node")
        self.force_always_listen = CONFIG.always_listen
        self.awaiting_command_after_wake = False
        self.surf_context = SurfContext()
        self.status: dict[str, Any] = {
            "pipeline": "surf_demo_workspace",
            "reply_backend": os.environ.get("DEMO_REPLY_BACKEND", "local"),
        ...
        self._update_status(
            service_state="ready",
            demo_server_url=CONFIG.demo_server_url,
            action_backend=CONFIG.action_backend,
            action_keyword_first=CONFIG.action_keyword_first,
        )
        self.get_logger().info("Demo SURF context node ready.")
        ...
        self.get_logger().info(f"Demo server: {CONFIG.demo_server_url}")
    ...
    def on_audio_msg(self, msg: String) -> None:
        ...
        if not self.force_always_listen:
            ...
            if command_text is None:
                if not self._consume_wake_listen_window():
                    self.get_logger().info("Second demo wake filter did not match; ignoring ASR text.")
                    return
                command_text = user_text.strip()
            elif not command_text:
                self._open_wake_listen_window()
                self.get_logger().info(
                    "Second demo wake filter matched. Waiting for the next ASR command event."
                )
                return
            user_text = command_text

        demo_text = self._build_demo_text(user_text)
        ...
        demo_response = self._request_demo(demo_text, session_id=request_session_id)
        demo_finished_at = time.time()
        reply = str(demo_response.get("reply", "")).strip()
        if not reply:
            self._update_status(last_error="demo_request_failed", updated_at=time.time())
            self._session_record("demo_failed", session_id=request_session_id)
            self._set_wake_light_blue()
            self._close_session("session_end")
            return

        self.get_logger().info(f"Demo reply: {reply}")
        timing = demo_response.get("timing", {})
        self._update_status(
            last_reply=reply,
            last_reply_time=demo_finished_at,
            latency={
                **self.status.get("latency", {}),
                "demo_ms": self._elapsed_ms(demo_started_at, demo_finished_at),
                "wake_to_reply_ms": self._elapsed_ms(self.surf_context.wake_time, demo_finished_at),
            },
        )
        self._session_record(
            "demo_reply",
    ...
    def _build_demo_text(self, user_text: str) -> str:
    ...
    def _request_demo(self, text: str, session_id: str = "default") -> dict[str, Any]:
        try:
            response = HTTP_SESSION.get(
                CONFIG.demo_server_url,
        ...
        if not reply:
            self.get_logger().error("Empty reply from Demo")
        result["reply"] = reply
        return result
    ...
    @staticmethod
    def _demo_tts_url() -> str:
        return CONFIG.demo_server_url.rsplit("/", 1)[0] + "/tts"
    ...
    def _convert_tts_to_wav(self) -> bool:
        if not CONFIG.tts_mp3_path.exists():
            self.get_logger().error(f"{CONFIG.tts_mp3_path} not found; Demo server did not generate it")
            return False
    ...
    def _run_reply_action_locked(self, reply: str, user_text: str = "") -> None:
        ...
        if payload is None:
            ...
            self._session_record("action_failed", reason="classifier_failed", reply=reply)
            return
