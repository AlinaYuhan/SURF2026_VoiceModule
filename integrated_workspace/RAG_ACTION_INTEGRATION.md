...
SURF ASR /audio_msg
  ...
  -> demo_server.py /infer
  -> xjtlu-rag-system /chat when DEMO_REPLY_BACKEND=rag
...
that topic for action execution. Instead, `demo_server.py` calls the RAG HTTP API
directly so the existing action path receives the final reply unchanged.

## Enable RAG

```bash
cd /home/louisxx/surf_demo_workspace
...
./scripts/env_set.sh OPENAI_API_KEY your-deepseek-api-key
...
```bash
${HOME}/.local/ollama/bin/ollama pull bge-m3
```
