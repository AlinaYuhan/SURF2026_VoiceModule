...
This document archives the SURF -> Demo -> Unitree G1 pipeline changes made on
2026-05-21.
...
- Mapped "胸前挥手" request to the closest official action:
  ...
  - Action ID: `25`
- Added configuration:
  - `SURF_DEMO_WAKE_ACK_ACTION_ENABLE=1`
  - `SURF_DEMO_WAKE_ACK_ACTION_ID=25`
  - `SURF_DEMO_WAKE_ACK_ACTION_LABEL=面前挥手`
...
- Added a short acknowledgement after valid ASR is accepted:
  - Text: `收到`
- Added configuration:
  - `SURF_DEMO_THINKING_ACK_ENABLE=1`
  - `SURF_DEMO_THINKING_ACK_TEXT=收到`
- Flow is now:

```text
wake word -> red light -> "我在" + face wave
command ASR accepted -> blinking green + "收到"
Demo/RAG/DeepSeek reply ready -> formal reply playback -> blue
```
...
journalctl --user -u surf-demo-node -f
journalctl --user -u surf-demo-audio-player -f
```
...
8. Demo/RAG/DeepSeek generates reply.
